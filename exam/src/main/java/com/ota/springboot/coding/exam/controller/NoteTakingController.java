...
public class NoteTakingController {
	...
	public  ResponseEntity<?> fetchNotesList(){
		...
		ArrayList<NotesEntity> listResult = this.notesService.findAllNotes();
		
		if(listResult.size() > 0) {
			return new ResponseEntity<ArrayList<NotesEntity>>(listResult, HttpStatus.OK);	
		}else {
			NoteResponse reponse = new NoteResponse();
			reponse.setStatus(HttpStatus.OK.value());
			reponse.setMessage("list is empty");
			reponse.setTimeStamp(System.currentTimeMillis());
			return new ResponseEntity<NoteResponse>(reponse, HttpStatus.OK);	
		}
	...
	public ResponseEntity<?> saveNotes(@RequestBody NotesEntity entity) throws Exception {
		...
		reponse.setTimeStamp(System.currentTimeMillis());
		return new ResponseEntity<NoteResponse>(reponse, HttpStatus.CREATED);	
	}
